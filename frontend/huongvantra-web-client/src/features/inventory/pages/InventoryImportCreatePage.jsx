...
import { createSupplierReceipt, getSupplierReceiptTemplateUrl, receiveSupplierReceipt } from '../services/supplierReceiptApi.js'
...
function InventoryImportCreatePage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const submitted = await receiveSupplierReceipt(receipt.id)
      showSuccess(`Đã nhận lô hàng ${submitted.receiptCode} vào kho. Phiếu đã chuyển cho kế toán thống kê.`)
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description="Thủ kho nhập chứng từ từ NCC. Nhận hàng là ghi nhận thẳng vào Kho, phiếu chuyển cho kế toán thống kê."
      />
      ...
      <form className="inventory-form grid grid-cols-1 gap-6" onSubmit={handleSubmit}>
        <section className="rounded-2xl bg-white p-6 shadow-sm sm:p-8">          <h2 className="mb-6 text-lg font-bold text-slate-800">Thông tin nhà cung cấp</h2>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            >
              {isSaving ? 'Đang nhận hàng...' : 'Nhận hàng vào kho'}
            </button>
            <Link
