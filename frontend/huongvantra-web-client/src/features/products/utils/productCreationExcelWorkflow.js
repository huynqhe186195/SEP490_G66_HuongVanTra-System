...
export function parseOfficialProductCreationWorkbook(workbook, context) {
  ...
  rawBomEntries.forEach(({ excelRow, productKey, values }) => {
    ...
    const baseSku = product.skuEntries.find((sku) => sku.isBaseUnitVariant)
    if (!outputSku.isBaseUnitVariant && baseSku && (
      normalizeSkuCode(baseSku.skuCode) === componentSkuCode ||
      normalizeComparable(baseSku.requestSkuKey) === normalizeComparable(componentSkuCode)
    )) {
      errors.push(`Dòng ${excelRow}: Không cần nhập BOM từ SKU quy đổi về SKU cơ bản. Hệ thống tự tạo dòng BOM bắt buộc theo Quy đổi.`)
      return
    }
