...
import {
  ...
  downloadOfficialProductCreationSample,
...
function usesCustomByDefault(productType) {
  return productType === PRODUCT_TYPE.NGUYEN_LIEU || productType === PRODUCT_TYPE.BAO_BI
}
...
function createSkuRow(row, { isBase = false } = {}) {
  ...
  return {
    ...
    canHaveBom: row.productType === PRODUCT_TYPE.THANH_PHAM ? true : false,
    canUseInCustom: usesCustomByDefault(row.productType),
    bomLines: [],
  }
}
...
const DECISION_LABELS = {
  Approved: 'Đã duyệt / tạo hàng hóa',
  Rejected: 'Từ chối',
...
function actorLabel(name, roleName) {
...
/**
 * Ghép timeline trạng thái từ request + revisions (mỗi lần gửi / quyết định).
 */
function buildRequestStatusTimeline(request) {
  if (!request) return []
  const events = []
  const revisions = Array.isArray(request.revisions)
    ? [...request.revisions].sort((a, b) => (a.revisionNumber || 0) - (b.revisionNumber || 0))
    : []
  ...
  if (request.createdAt) {
    events.push({
      ...
      by: actorLabel(request.createdByName, request.createdByRoleName),
      reason: '',
      tone: 'draft',
    })
  }
  ...
  if (revisions.length === 0) {
    ...
    if (request.reviewedAt || request.rejectReason || request.cancelReason || ['Completed', 'Rejected', 'Cancelled'].includes(request.status)) {
      ...
      events.push({
        ...
        at: request.reviewedAt || request.completedAt || request.updatedAt,
        by: actorLabel(request.reviewedByName, request.reviewedByRoleName),
        reason: request.rejectReason || request.cancelReason || request.adminNote || '',
        tone,
      })
    }
    return events
  ...
  revisions.forEach((revision) => {
    const revNo = revision.revisionNumber || 0
    if (revision.submittedAt) {
      events.push({
        id: `rev-${revision.id || revNo}-submit`,
        label: revisions.length > 1 ? `Gửi duyệt (lần ${revNo})` : 'Gửi duyệt',
        at: revision.submittedAt,
        by: actorLabel(revision.submittedByName, revision.submittedByRoleName),
        reason: '',
        tone: 'pending',
      })
    }
    if (revision.decision) {
      events.push({
        id: `rev-${revision.id || revNo}-decision`,
        label: DECISION_LABELS[revision.decision] || revision.decision,
        at: revision.decidedAt,
        by: actorLabel(revision.decidedByName, revision.decidedByRoleName),
        reason: revision.decisionReason || '',
        tone: decisionTone(revision.decision),
      })
    }
  })

  if (request.status === 'Completed' && request.completedAt) {
    const hasApproved = events.some((event) => event.tone === 'approved')
    if (!hasApproved) {
      events.push({
        id: 'completed',
...
function RequestStatusHistory({ request, compact = false }) {
  const events = buildRequestStatusTimeline(request)
  if (events.length === 0) {
    return (
      <div className="space-y-1">
        <StatusChip status={request?.status} />
        <p className="text-[11px] text-slate-400">Chưa có lịch sử trạng thái.</p>
      </div>
    )
  }

  return (
    <div className={compact ? 'min-w-[220px] max-w-[320px]' : 'w-full'}>
      {!compact ? (
        ...
        <div className="mb-1.5"><StatusChip status={request.status} /></div>
      )}
      <ol className={`relative space-y-2 border-l border-slate-200 ${compact ? 'ml-1.5 pl-3' : 'ml-2 pl-4'}`}>
        {events.map((event) => (
          <li key={event.id} className="relative">
            <span className={`absolute -left-[19px] top-1.5 h-2.5 w-2.5 rounded-full ring-2 ring-white ${TIMELINE_DOT_CLASS[event.tone] || TIMELINE_DOT_CLASS.draft}`} />
            <div className={`rounded-lg border px-2 py-1.5 ${TIMELINE_TONE_CLASS[event.tone] || TIMELINE_TONE_CLASS.draft}`}>
              <p className={`font-semibold ${compact ? 'text-[11px] leading-4' : 'text-xs'}`}>{event.label}</p>
              <p className={`opacity-80 ${compact ? 'text-[10px] leading-4' : 'text-[11px]'}`}>
                {formatDateTimeVN(event.at) || '—'}
                {event.by ? ` · ${event.by}` : ''}
              </p>
              {event.reason ? (
                <p className={`mt-0.5 opacity-90 ${compact ? 'text-[10px] leading-4' : 'text-[11px]'}`}>
                  Lý do: {event.reason}
                </p>
              ) : null}
            </div>
...
function getBomLineDisplayName(line) {
  const productName = line?.materialName || line?.MaterialName || ''
  const variantName = line?.componentVariantName || line?.ComponentVariantName || ''
  if (productName && variantName && !normalizeText(variantName).toLowerCase().includes(normalizeText(productName).toLowerCase())) {
    return `${productName} · ${variantName}`
  }
  return productName || variantName || line?.componentSkuCode || line?.ComponentSkuCode || line?.materialId || '—'
}
...
function getManualBomLines(sku) {
  return (sku?.bomLines ?? []).filter((line) => line?.isRequiredBaseComponent !== true)
}

function hasRequiredFinishedProductBom(skuRows = []) {
  return skuRows.some((sku) => {
    if (!sku?.isBaseUnitVariant) return false
    return getManualBomLines(sku).some((line) => Boolean(getBomComponentKey(line) || line.materialId))
  })
}
...
function syncRequiredBaseBomLines(row, skuRows) {
  ...
  return normalizedRows.map((sku, index) => {
    ...
    const manualLines = (sku.bomLines ?? []).filter((line) => line.isRequiredBaseComponent !== true)
...
function syncSkuRows(row, skuRows) {
  ...
  const withDirectCosts = withBomLines.map((sku) => {
    const bomCost = Math.round(calculateSkuBomCost(sku, withBomLines))
    const importedCost = moneyOrNull(sku.costPrice) ?? 0
    return {
      ...sku,
      costPrice: String(bomCost > 0 ? bomCost : importedCost),
    }
  })

  return withDirectCosts.map((sku) => {
    const bomCost = Math.round(calculateSkuBomCost(sku, withDirectCosts))
    const importedCost = moneyOrNull(sku.costPrice) ?? 0
    return {
      ...sku,
      costPrice: String(bomCost > 0 ? bomCost : importedCost),
  ...
  return {
    ...
    variants: skuRows.map((sku) => {
      ...
      return {
        ...
        isPurchasable: sku.isPurchasable !== false,
        canBeBomComponent: isFinishedProduct ? sku.canBeBomComponent === true : sku.canBeBomComponent !== false,
        canUseInCustom: sku.canUseInCustom === true,
        canHaveBom: isFinishedProduct ? true : false,
        units: [{
          unitName: sku.unitName || baseUnit,
          conversionRate: 1,
        ...
        bomLines: isFinishedProduct && sku.isBaseUnitVariant
          ? getManualBomLines(sku)
...
function fromProductSnapshot(item, materialsById) {
  ...
  const skuRows = variants.map((variant, index) => {
    ...
    return {
      ...
      canBeBomComponent: variant.canBeBomComponent === true,
      canUseInCustom: variant.canUseInCustom === true,
      canHaveBom: variant.canHaveBom !== false,
...
function validateForm(title, rows, options = {}) {
  ...
  rows.forEach((row, index) => {
    ...
    if (requireFinishedBom && isFinishedProduct && !hasRequiredFinishedProductBom(skuRows)) {
      errors.push(`${prefix}: Sản phẩm kệ bắt buộc phải có BOM trước khi gửi duyệt.`)
    }
    ...
    skuRows.forEach((sku, skuIndex) => {
      ...
      if (sku.isBaseUnitVariant && (normalizeText(sku.baseRequestSkuKey) || normalizeText(sku.baseSkuCode))) errors.push(`${skuPrefix}: SKU đơn vị cơ bản không được tham chiếu SKU cơ bản khác.`)
      if (!sku.isBaseUnitVariant && !(normalizeText(sku.baseRequestSkuKey) || normalizeText(sku.baseSkuCode))) errors.push(`${skuPrefix}: SKU quy đổi phải tham chiếu SKU đơn vị cơ bản.`)
      if (!sku.isBaseUnitVariant && (sku.bomLines ?? []).length > 0) errors.push(`${skuPrefix}: BOM của SKU quy đổi được hệ thống tự động tính từ SKU đơn vị cơ bản.`)
...
function ProductRow({ row, rowIndex, categories, materials, existingProducts, attributeNameOptions, onChange, onImagesChange, onRemove, canRemove }) {
  ...
  function updateProduct(changes) {
    ...
    const shouldEnableCustom = changes.productType && usesCustomByDefault(changes.productType)
    const nextSkuRows = skuRows.map((sku) => {
      ...
      return {
        ...
        canUseInCustom: shouldEnableCustom ? true : sku.canUseInCustom,
      }
    })
  ...
  function getAvailableComponentOptions(skuIndex) {
    ...
    const localOptions = row.productType === PRODUCT_TYPE.THANH_PHAM ? [] : skuRows
    ...
    const externalOptions = componentSkuOptions
      ...
      .filter((option) => {
        ...
        return [PRODUCT_TYPE.NGUYEN_LIEU, PRODUCT_TYPE.BAO_BI, PRODUCT_TYPE.THANH_PHAM].includes(option.productType)
          && option.canBeBomComponent === true
          && optionSku !== normalizeText(sku?.skuCode).toUpperCase()
        ...
        <div className="mt-3 space-y-3">
          {skuRows.map((sku, index) => (
            <div key={sku.requestSkuKey || index} className="rounded-lg border border-slate-200 bg-white p-3">
              <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
                <div className="flex min-w-0 items-center gap-2">
                  <span className="truncate text-xs font-bold uppercase tracking-wide text-slate-700">
                    {sku.isBaseUnitVariant ? 'Đơn vị cơ bản' : `Đơn vị ${index + 1}`}
                  </span>
                  {sku.unitName ? <span className="truncate text-xs font-semibold text-slate-500">— {sku.unitName}</span> : null}
                </div>
                <button
                  type="button"
                  disabled={sku.isBaseUnitVariant}
                  className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-rose-600 hover:text-rose-700 disabled:opacity-30"
              ...
              <p className="mt-3 text-[11px] font-bold uppercase tracking-wide text-slate-400">Định danh</p>
              <div className="mt-1.5 grid items-start gap-3 md:grid-cols-6">
              ...
              <p className="mt-4 text-[11px] font-bold uppercase tracking-wide text-slate-400">Giá & thuế</p>
              <div className="mt-1.5 grid items-start gap-3 md:grid-cols-4">
                <label className="text-xs font-semibold text-slate-500">
                  Giá trước thuế
              ...
              <p className="mt-4 text-[11px] font-bold uppercase tracking-wide text-slate-400">Nhận diện & ngưỡng tồn</p>
              <div className="mt-1.5 grid items-start gap-3 md:grid-cols-6">
                <label className="text-xs font-semibold text-slate-500 md:col-span-2">
                  Barcode
                ...
                <div className="mt-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
                      BOM cho SKU {sku.skuCode || index + 1}
                      {sku.isBaseUnitVariant ? <span className="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-800">Bắt buộc</span> : null}
                    </p>
                    ...
                    <div className="mt-3 overflow-x-auto">
                      <table className="min-w-full text-left text-xs">
                        ...
                        <tbody className="divide-y divide-slate-200">
                          {(sku.isBaseUnitVariant ? getManualBomLines(sku) : deriveVariantBomPreview((skuRows.find((item) => item.isBaseUnitVariant) ?? {}).bomLines, sku.conversionRate)).map((line, lineIndex) => (
                            <tr key={`${line.componentVariantId || line.componentRequestSkuKey || line.materialId}-${lineIndex}`} className={line.isRequiredBaseComponent ? 'bg-emerald-50/45' : ''}>
                              <td className="py-2 pr-3">
                                <div className="flex flex-wrap items-center gap-2">
                                  <p className="font-semibold text-slate-700">{getBomLineDisplayName(line)}</p>
                              ...
                              <td className="py-2 pr-3">
                                <div className="flex items-center gap-2">
                                  <input type="text" inputMode="numeric" pattern="[0-9]*" disabled={!sku.isBaseUnitVariant} onKeyDown={preventNonIntegerKey} className="w-28 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm disabled:border-indigo-100 disabled:bg-indigo-50 disabled:font-semibold disabled:text-indigo-800" value={line.quantity} onChange={(event) => updateBomLine(index, lineIndex, { quantity: event.target.value })} />
                                  <span className="text-slate-500">{line.materialUnitName || 'đơn vị'}</span>
                                </div>
                              </td>
                              <td className="py-2 text-right">
                                {!sku.isBaseUnitVariant ? (
                                  <span className="text-xs font-semibold text-indigo-500">Hệ thống tính</span>
                                ) : (
      ...
      <div className="flex max-h-[min(92dvh,880px)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="custom-scrollbar flex-1 space-y-4 overflow-y-auto p-6">
          <section className="grid gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm md:grid-cols-3">
            ...
            <div><p className="text-xs font-bold uppercase text-slate-400">Trạng thái hiện tại</p><StatusChip status={request.status} /></div>
          ...
          <section className="rounded-xl border border-slate-100 bg-white p-4">
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Lịch sử trạng thái</p>
            <div className="mt-3">
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="w-[480px] max-w-[480px] px-4 py-3">Tiêu đề</th>
              <th className="px-4 py-3">Người gửi</th>
              <th className="px-4 py-3">Thời gian gửi</th>
              <th className="py-3 pl-0 pr-4">Số sản phẩm</th>
              <th className="px-4 py-3">Lịch sử trạng thái</th>
            ...
            {requests.map((request) => (
              <tr key={request.id} className="align-top hover:bg-slate-50/70">
                ...
                <td className="max-w-[480px] px-4 py-3">
                  <p className="line-clamp-2 font-semibold text-slate-800">{request.title}</p>
                ...
                <td className="py-3 pl-0 pr-4 font-semibold">{request.items.length}</td>
                <td className="px-4 py-3">
                  <RequestStatusHistory request={request} compact />
                </td>
...
export default function ProductApprovalsPage() {
  ...
  async function handleSubmit() {
    ...
    const postUploadErrors = validateForm(title, uploadedRows, { requireFinishedBom: true })
    if (postUploadErrors.length) {
      showError(postUploadErrors[0])
      setIsSaving(false)
      return
    }
  ...
  async function downloadSampleWithData() {
    setIsTemplateGenerating(true)
    try {
      const { categoryItems, materialItems, attributeNameItems, failures, referenceLoadStatuses } = await loadExcelReferenceData()
      await downloadOfficialProductCreationSample({
        categories: categoryItems,
        materials: materialItems,
        attributeNameOptions: buildAttributeNameOptions(attributeNameItems, []),
        referenceLoadStatuses,
      ...
      showError('Không tạo được file có dữ liệu mẫu. Vui lòng thử lại.')
    } finally {
      setIsTemplateGenerating(false)
    }
  }
  ...
  async function exportCurrentDraft() {
    const exportableRows = rows.filter((row) => normalizeText(row.name) || (getSkuRows(row).some((sku) => normalizeText(sku.skuCode) || normalizeText(sku.unitName))))
    if (!exportableRows.length) {
      showError('Chưa có dữ liệu sản phẩm để export. Hãy điền ít nhất tên sản phẩm hoặc SKU.')
      return
    }
    ...
    try {
      ...
      await exportProductCreationDraftToTemplate({
        ...
        categories: categoryItems,
        materials: materialItems,
        attributeNameOptions: buildAttributeNameOptions(attributeNameItems, exportableRows),
        referenceLoadStatuses,
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                value={requestSearch}
                onChange={(e) => { setRequestSearch(e.target.value); setListPage(1) }}
              />
            </div>
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!isLoading && filteredRequests.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">{requestSearch ? 'Không tìm thấy yêu cầu phù hợp.' : 'Chưa có yêu cầu tạo hàng hóa.'}</td></tr>
              ) : null}
              {pagedRequests.map((request) => (
                <tr key={request.id} className="align-top">
                  <td className="px-4 py-3 font-mono text-xs text-[#356647]">{request.requestCode}</td>
                  <td className="max-w-[480px] px-4 py-3 font-semibold text-slate-800">
                  ...
                  <td className="py-3 pl-0 pr-4 font-semibold text-slate-800">{request.items.length}</td>
                  <td className="px-4 py-3">
                    <RequestStatusHistory request={request} compact />
                  </td>
                  <td className="px-4 py-3 text-xs text-slate-500">{formatDateTimeVN(request.submittedAt || request.createdAt)}</td>
