  ...
  return {
    ...
    variants: skuRows.map((sku) => {
      ...
      return {
        ...
        canHaveBom: isFinishedProduct ? sku.canHaveBom !== false : false,
        units: [{
          unitName: sku.unitName || baseUnit,
          conversionRate: 1,
        ...
        bomLines: isFinishedProduct && sku.isBaseUnitVariant
          ? (sku.bomLines ?? [])
            .filter((line) => line.isRequiredBaseComponent !== true)
...
function validateForm(title, rows, options = {}) {
  ...
  rows.forEach((row, index) => {
    ...
    if (requireFinishedBom && isFinishedProduct && !skuRows.some((sku) => sku.isBaseUnitVariant && (sku.bomLines ?? []).length > 0)) {
            ...
            <div key={sku.requestSkuKey || index} className="rounded-lg border border-slate-200 bg-white p-3">
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-6">
                ...
                {isFinishedProduct ? (
                  <>
                    <label className="flex items-center gap-2 pt-6 text-sm font-semibold text-slate-700">
                      <input type="checkbox" checked={sku.canHaveBom !== false} onChange={(event) => updateSku(index, { canHaveBom: event.target.checked })} />
                      Có thể có BOM
                    </label>
                    <label className="flex items-center gap-2 pt-6 text-sm font-semibold text-slate-700">
              ...
              {isFinishedProduct ? (
                <div className="mt-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
                  ...
                  {((sku.isBaseUnitVariant ? sku.bomLines : deriveVariantBomPreview((skuRows.find((item) => item.isBaseUnitVariant) ?? {}).bomLines, sku.conversionRate)) ?? []).length === 0 ? <p className="mt-3 text-xs text-slate-400">Chưa có component BOM.</p> : null}
                  {((sku.isBaseUnitVariant ? sku.bomLines : deriveVariantBomPreview((skuRows.find((item) => item.isBaseUnitVariant) ?? {}).bomLines, sku.conversionRate)) ?? []).length > 0 ? (
                    <div className="mt-3 overflow-x-auto">
                      <table className="min-w-full text-left text-xs">
                        ...
                        <tbody className="divide-y divide-slate-200">
                          {(sku.isBaseUnitVariant ? sku.bomLines : deriveVariantBomPreview((skuRows.find((item) => item.isBaseUnitVariant) ?? {}).bomLines, sku.conversionRate)).map((line, lineIndex) => (
