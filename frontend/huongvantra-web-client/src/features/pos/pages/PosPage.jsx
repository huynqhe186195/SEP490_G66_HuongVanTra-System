...
function PosPage() {
    ...
    }, [appliedPromotion, cartItems, effectiveOrderDiscountAmountFixed, effectiveOrderDiscountPercent]);
    ...
    const formatPromotionDiscountText = (promotion) =>
        promotion.discountType === "FIXED" ? `Giảm ${formatMoney(promotion.discountValue)}đ` : `Giảm ${promotion.discountValue}%`;
                                    ...
                                    <div className="flex min-w-max gap-2">
                                        <button
                                            ...
                                            className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${
                                                !selectedCategoryId ? "bg-[#356647] text-white" : "bg-white text-[#414942] hover:bg-[#f0eee6]"
                                            }`}>
                                            Tất cả
                                        </button>
                                        {posCategories.map((category) => (
                                            <button
                                                ...
                                                className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${
                                                    selectedCategoryId === String(category.id) ? "bg-[#356647] text-white" : "bg-white text-[#414942] hover:bg-[#f0eee6]"
                                                }`}>
                                                {category.name}
                                            </button>
                                        ))}
                            ...
                            <div className="custom-scrollbar flex-1 overflow-y-auto px-3 py-3">
                                ...
                                : filteredSearchProducts.length === 0 ?
                                    <p className="px-1 py-3 text-sm text-[#717971]">
                                        {hasSearchQuery || selectedCategoryId ? "Không tìm thấy sản phẩm phù hợp." : "Chưa có sản phẩm để hiển thị."}
                                    </p>
