    ...
    IOptions<InventoryOptions> inventoryOptions)
{
    ...
    private const string DefectReasonOther = "OTHER";
    private static readonly IReadOnlyDictionary<string, string> SupplierReturnDefectReasonCodes = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
        ["DAMAGED_ON_ARRIVAL"] = "Hàng hỏng/vỡ khi nhận",
        ["EXPIRED_OR_NEAR_EXPIRY"] = "Hết hạn hoặc cận hạn",
        ["QUALITY_NOT_MET"] = "Không đạt chất lượng",
        ["WRONG_ITEM_DELIVERED"] = "Giao sai hàng",
        ["PACKAGING_DEFECT"] = "Lỗi bao bì",
        ["CONTAMINATED_OR_MOLDY"] = "Ẩm mốc/nhiễm bẩn",
        ["OTHER"] = "Khác",
    };
    ...
    private static readonly Regex SupplierLotCodeRegex = new(@"^[A-Za-z0-9\-_]{1,50}$", RegexOptions.Compiled);
    ...
    public async Task HandleOrderPlacedAsync(OrderPlacedEvent message, CancellationToken ct = default)
    {
        ...
        if (existingQueue != null)
        {
            ...
            // Đơn đã thanh toán/hoàn tất → tự trừ tồn quầy (chỉ queue thuần Kệ / legacy).
            // Sell-first (điều chuyển / BOM / backorder) giữ Waiting để Thủ kho Confirm.
            if (IsPaidOrderPaymentStatus(paymentStatus)
                && existingQueue.QueueStatus == QueueStatus.Waiting
                && !existingQueue.IsDeducted
                && ShouldAutoConfirmOnPaidOrderPlaced(existingQueue))
            {
        ...
        if (IsPaidOrderPaymentStatus(paymentStatus) && ShouldAutoConfirmOnPaidOrderPlaced(queue))
    ...
    /// <summary>
    /// Chỉ auto-confirm khi thu tiền nếu queue không cần Thủ kho (sell-first).
    /// </summary>
    private static bool ShouldAutoConfirmOnPaidOrderPlaced(StockDeductQueue queue)
    {
        var status = (queue.OrderStockStatus ?? string.Empty).Trim().ToLowerInvariant();
        return status is not (
            "pending_warehouse_transfer"
            or "pending_bom_reconciliation"
            or "waiting_materials");
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var response = await _unitOfWork.ExecuteInTransactionAsync(async innerCt =>
        {
            ...
            {
                // HasPending = cần Thủ kho (điều chuyển / sản xuất / backorder), không gồm COD chỉ giữ chỗ Kệ.
                var previewNeedsThuKho = pendingDecisions.Count > 0
                    || decisions.Any(d => d.WarehouseDeductedQuantity > 0)
                    || shortages.Count > 0;
                var previewMode = shortages.Count > 0
                    ? "BackorderAccepted"
                    : pendingDecisions.Count > 0
                        ? "PreviewPendingBomReconciliation"
                        : decisions.Any(d => d.WarehouseDeductedQuantity > 0)
                            ? "PreviewWarehouseTransfer"
                            : "PreviewImmediate";
                return new PosStockHandlingResponse(
                    request.OrderId,
                    request.OrderCode.Trim(),
            ...
            // POS: queue cho WH transfer + BOM. COD (ReserveOnly): luôn tạo queue (cả khi chỉ Kệ)
            // để giữ chỗ đến lúc Shipping/Confirm.
            var queueDecisions = request.ReserveOnly
                ? decisions
                : decisions
            ...
            if (queueDecisions.Count > 0)
            {
                var queue = new StockDeductQueue
                {
                    ...
                    Items = queueDecisions.Select(d => new StockDeductQueueItem
                    {
                        ...
                        // COD ReserveOnly: Quantity = full order (Confirm xuất đủ + nhả reserved).
                        // POS: Quantity = phần chưa trừ ngay (WH + BOM).
                        Quantity = request.ReserveOnly
                            ? d.OrderedQuantity
                            : d.PendingBomQuantity + d.WarehouseDeductedQuantity,
                ...
                if (request.ReserveOnly)
                {
                    var shelfReserveBySku = queueDecisions
                        .GroupBy(d => d.SkuId)
                        .ToDictionary(g => g.Key, g => g.Sum(x => x.FinishedDeductedQuantity));

                    foreach (var (skuId, shelfQty) in shelfReserveBySku.OrderBy(kv => kv.Key))
                    {
                        if (shelfQty <= 0) continue;
                        var decision = queueDecisions.First(d => d.SkuId == skuId);
                        var stock = decision.Stock
                            ?? await GetOrCreateSkuStockAsync(skuId, decision.SkuCode, innerCt);
                        stock.ReservedQuantity += shelfQty;
                        stock.UpdatedAt = now;
                    }

                    queue.IsReserved = shelfReserveBySku.Values.Any(v => v > 0);
            ...
            var needsThuKho = pendingDecisions.Count > 0
                || decisions.Any(d => d.WarehouseDeductedQuantity > 0)
                || shortages.Count > 0;
            return new PosStockHandlingResponse(
                request.OrderId,
        ...
        if (!request.PreviewOnly && !response.BackorderRequired && !request.ReserveOnly)
        {
        ...
        CancellationToken ct)
    {
        return await _unitOfWork.ExecuteInTransactionAsync(async innerCt =>
        {
        var queue = await _queueRepo.GetByOrderIdAsync(orderId, innerCt);
        if (queue is null)
        {
        ...
        if (queue.QueueStatus == QueueStatus.Cancelled)
        {
            ...
            if (queue.IsReserved)
            {
                await ReleaseQueueReservationAsync(queue, innerCt);
                await _queueRepo.SaveChangesAsync(innerCt);
        ...
        if (queue.QueueStatus == QueueStatus.Confirmed || queue.IsDeducted)
        {
            ...
            await RestoreStockAsync(queue, innerCt);
        ...
        // Waiting / Insufficient (POS WaitingTransfer / WaitingProduction / Backorder):
        // nhả giữ / hoàn phần Kệ đã trừ trước khi đánh dấu Cancelled. Nếu thất bại,
        // rollback toàn bộ để OrderCancelledEvent còn retry được.
    ...
    public async Task<StockDeductPreviewResponse> PreviewQueueAsync(Guid queueId, CancellationToken ct = default)
    {
        ...
        // COD/legacy: gắn BOM snapshot khi Kệ+Kho FG không đủ — để modal «Xem» báo sẽ sinh lệnh SX như POS.
        var stockBySkuId = new Dictionary<Guid, SkuStock>();
        foreach (var item in queue.Items)
        {
            if (item.SkuId == Guid.Empty) continue;
            var stock = await _skuStockRepo.GetBySkuIdWithLockAsync(item.SkuId, ct)
                ?? await GetOrCreateSkuStockAsync(item.SkuId, item.SkuSnapshotCode, ct);
            stockBySkuId[item.SkuId] = stock;
        }

        if (await EnrichQueueSellFirstBomAsync(queue, stockBySkuId, DateTime.UtcNow, ct))
            await _queueRepo.SaveChangesAsync(ct);
    ...
    /// <summary>
    /// COD / queue legacy: nếu Kệ + Kho thành phẩm không đủ phần đặt, gắn PendingBom + snapshot NL
    /// giống PreparePos — để Confirm tự sinh lệnh SX + phiếu điều chuyển như POS.
    /// </summary>
    /// <returns>true nếu metadata queue đã đổi (cần SaveChanges).</returns>
    private async Task<bool> EnrichQueueSellFirstBomAsync(
        StockDeductQueue queue,
        Dictionary<Guid, SkuStock> stockBySkuId,
        DateTime now,
        CancellationToken ct)
    {
        // Đã có snapshot BOM đầy đủ → Confirm sẽ đi đúng nhánh sản xuất.
        if (IsBomReconciliationQueue(queue))
            return false;

        var changed = false;
        var shelfReserveBySku = new Dictionary<Guid, int>();
        var decisionsNeedingBom = new List<(StockDeductQueueItem Item, PosStockDecision Decision)>();
        ...
        foreach (var item in queue.Items.OrderBy(i => i.SkuId))
        {
            ...
            // Đã có PendingBom + snapshot từng dòng → giữ nguyên.
            ...
            if (item.FinishedDeductedQuantity != split.FromShelf
                || item.WarehouseTransferQuantity.GetValueOrDefault() != split.FromWarehouse
                || item.PendingBomQuantity.GetValueOrDefault() != split.StillShort)
            {
                item.FinishedDeductedQuantity = split.FromShelf;
                item.WarehouseTransferQuantity = split.FromWarehouse;
                item.PendingBomQuantity = split.StillShort;
                changed = true;
            }
            ...
            if (split.StillShort <= 0)
            {
                ...
                {
                    item.MaterialRequirementSnapshotJson = null;
                    changed = true;
                }
            ...
            decisionsNeedingBom.Add((item, new PosStockDecision
            {
                SkuId = item.SkuId,
                SkuCode = item.SkuSnapshotCode,
                SkuName = item.SkuSnapshotName,
                OrderedQuantity = item.Quantity,
                FinishedDeductedQuantity = split.FromShelf,
                WarehouseDeductedQuantity = split.FromWarehouse,
                PendingBomQuantity = split.StillShort,
                Stock = stock,
            }));
        }

        if (shelfReserveBySku.Count > 0 && shelfReserveBySku.Values.Any(v => v > 0))
        {
            StampItemsReservedPartial(queue, shelfReserveBySku, now);
            changed = true;
        }

        if (decisionsNeedingBom.Count == 0)
        {
            if (queue.Items.Any(i => i.WarehouseTransferQuantity.GetValueOrDefault() > 0)
                && !string.Equals(queue.OrderStockStatus, "pending_warehouse_transfer", StringComparison.OrdinalIgnoreCase)
        ...
        {
            catalog = await _productCatalogClient.GetCatalogForVariantIdsAsync(
                decisionsNeedingBom.Select(x => x.Decision.SkuId),
                ct);
        }
        catch (InventoryValidationException)
        {
            throw new InventoryValidationException(
                "Không tải được dữ liệu BOM để xác nhận trừ hàng COD. Vui lòng thử lại hoặc kiểm tra kết nối dịch vụ Product.");
        }

        var decisions = decisionsNeedingBom.Select(x => x.Decision).ToList();
        ValidatePendingPosCatalog(decisions, catalog);
        // acceptBackorder=true: luôn gắn snapshot dù NL thiếu — Confirm sẽ đánh Insufficient như POS.
        await ResolvePendingBomMaterialSnapshotsAsync(decisions, catalog, acceptBackorder: true, ct);

        foreach (var (item, decision) in decisionsNeedingBom)
        {
            item.PendingBomQuantity = decision.PendingBomQuantity;
            item.FinishedDeductedQuantity = decision.FinishedDeductedQuantity;
            item.WarehouseTransferQuantity = decision.WarehouseDeductedQuantity;
            item.MaterialRequirementSnapshotJson = decision.MaterialRequirements.Count > 0
                ? JsonSerializer.Serialize(decision.MaterialRequirements, MaterialSnapshotJsonOptions)
                : null;
            item.StockHandlingMode = ResolveQueueItemHandlingMode(decision, fulfillmentPreference: null);
            changed = true;
        }

        if (!string.Equals(queue.OrderStockStatus, OrderStockStatusCancellationRequested, StringComparison.OrdinalIgnoreCase))
            queue.OrderStockStatus = "pending_bom_reconciliation";
        ...
        CancellationToken ct = default)
    {
        ...
        var result = await _unitOfWork.ExecuteInTransactionAsync(async innerCt =>
        {
            ...
            // COD sell-first: trước Confirm, gắn BOM + snapshot NL nếu Kệ+Kho FG không đủ
            // (queue tạo qua OrderPlaced/reserve cũ không có metadata POS-06).
            await EnrichQueueSellFirstBomAsync(queue, stockBySkuId, now, innerCt);
            ...
            foreach (var item in orderedItems)
            {
                ...
                // COD có bù Kho: chỉ nhả ReservedQuantity (phần Kệ), không dùng item.Quantity.
                if (queue.IsReserved)
                    stock.ReservedQuantity = Math.Max(0, stock.ReservedQuantity - Math.Max(0, item.ReservedQuantity));
                stock.UpdatedAt = now;
            }
    ...
    private async Task ReleaseOrRestoreQueueHoldAsync(StockDeductQueue queue, CancellationToken ct)
    {
        ...
        // COD ReserveOnly cũng lưu FinishedDeductedQuantity để thể hiện phần Kệ đã
        // được giữ chỗ. Giá trị đó là kế hoạch phân bổ, không có nghĩa hàng đã
        // xuất. Queue có IsReserved phải chỉ nhả reservation; nếu không sẽ cố tìm
        // phiếu xuất POS không tồn tại và rollback toàn bộ thao tác hủy.
        var hasBackorderHold = !queue.IsReserved && queue.Items.Any(HasImmediateShelfHold);
        ...
        CreatorSnapshot? creator = null)
    {
        ...
        await GuardDuplicateShelfReplenishmentAsync(request.Items, ct);
    ...
    /// <summary>
    /// Chặn gửi trùng SKU khi yêu cầu trước đó chưa xử lý xong.
    /// Xét theo từng dòng, không theo trạng thái phiếu, vì một phiếu đang xử lý dở
    /// có thể vừa có dòng đã chốt vừa có dòng còn treo.
    /// Mỗi SKU chỉ tạo lại được sau khi dòng SKU của yêu cầu cũ đã kết thúc xử lý.
    /// </summary>
    private async Task GuardDuplicateShelfReplenishmentAsync(
        List<CreateStockAdjustmentRequestItem> items,
        CancellationToken ct)
    {
        var duplicates = await FindOpenDuplicateShelfSkusAsync(items.Select(i => i.SkuId), ct);
        if (duplicates.Count == 0) return;

        throw new DuplicateStockAdjustmentRequestException(
            BuildBlockingDuplicateMessage(duplicates),
            duplicates,
            blocking: true);
    }

    /// <summary>
    /// Kiểm tra trùng mà không gửi yêu cầu, để màn hình tạo yêu cầu chặn sớm
    /// thay vì để người dùng xác nhận xong mới nhận lỗi.
    /// </summary>
    public async Task<StockAdjustmentDuplicateCheckResponse> CheckStockAdjustmentDuplicatesAsync(
        IEnumerable<Guid> skuIds,
        CancellationToken ct = default)
    {
        var duplicates = await FindOpenDuplicateShelfSkusAsync(skuIds, ct);
        if (duplicates.Count > 0)
        {
            return new StockAdjustmentDuplicateCheckResponse(
                true,
                false,
                BuildBlockingDuplicateMessage(duplicates),
                duplicates.Select(MapDuplicateSku).ToList());
        }

        return new StockAdjustmentDuplicateCheckResponse(false, false, null, []);
    }

    private async Task<List<DuplicateStockAdjustmentSku>> FindOpenDuplicateShelfSkusAsync(
        IEnumerable<Guid> skuIds,
        CancellationToken ct)
    {
        var openItems = await _adjustmentRequestRepo.GetOpenItemsBySkuIdsAsync(skuIds, ct);

        return openItems
            .Select(line => new DuplicateStockAdjustmentSku(
                line.SkuId,
                line.SkuCode,
                line.SkuSnapshotName,
                line.RequestId,
                line.Request?.RequestCode ?? string.Empty,
                (line.Request?.Status ?? StockAdjustmentRequestStatus.Pending).ToString(),
                line.Status.ToString(),
                line.Request?.RequestedAt ?? DateTime.UtcNow,
                line.Request?.RequestedByName,
                line.RemainingQuantity,
                line.Status == StockAdjustmentRequestItemStatus.Pending
                    && line.ApprovedQuantity == 0
                    && line.FulfilledQuantity == 0))
            .ToList();
    }

    // Chỉ nêu tên sản phẩm; mã phiếu để phần chi tiết bên dưới liệt kê, tránh lặp.
    private static string BuildBlockingDuplicateMessage(List<DuplicateStockAdjustmentSku> blocking) =>
        $"{string.Join(", ", blocking.Select(d => d.SkuSnapshotName).Distinct())} "
        + "đang có dòng yêu cầu chưa xử lý xong ở phiếu trước. "
        + "Vui lòng hoàn tất, từ chối hoặc hủy dòng cũ trước khi gửi yêu cầu mới.";

    private static StockAdjustmentDuplicateSkuResponse MapDuplicateSku(DuplicateStockAdjustmentSku d) =>
        new(
            d.SkuId,
            d.SkuCode,
            d.SkuSnapshotName,
    ...
    /// <summary>
    /// Điểm tương thích nội bộ cho test và dữ liệu cũ. WebAPI không còn công khai bước duyệt này;
    /// luồng mới phải đi qua ShelfReplenishmentWorkflowLogic.ProcessItemAsync.
    /// </summary>
    [Obsolete("Luồng mới xử lý trọn sản phẩm qua ShelfReplenishmentWorkflowLogic.ProcessItemAsync.")]
    public async Task<StockAdjustmentRequestResponse> ReviewStockAdjustmentRequestAsync(
        Guid id,
        Guid reviewedBy,
        object? legacyRequest,
        CancellationToken ct = default,
        CreatorSnapshot? reviewer = null)
    {
        ...
        if (entity.Status != StockAdjustmentRequestStatus.Pending)
            throw new InventoryValidationException("Yêu cầu không còn ở trạng thái chờ xử lý.");

        await ValidateShelfReplenishmentCatalogAsync(entity.Items, ct);
        foreach (var line in entity.Items.Where(item => !StockAdjustmentFulfillment.IsLineClosed(item.Status)))
        {
            line.ApprovedQuantity = line.QuantityDelta;
        ...
        entity.ReviewedByName = reviewer?.CreatedByName;
        entity.ReviewedByRoleName = reviewer?.CreatedByRoleName;
        entity.ReviewedAt = DateTime.UtcNow;
        await _adjustmentRequestRepo.SaveChangesAsync(ct);
        return MapAdjustmentRequest(entity);
    }

    /// <summary>Manager hủy yêu cầu khi Warehouse chưa bắt đầu xử lý sản phẩm nào.</summary>
        ...
        CreatorSnapshot? actor = null)
    {
        ...
        if (entity.Status is not (StockAdjustmentRequestStatus.Pending or StockAdjustmentRequestStatus.Draft))
            throw new InventoryValidationException("Chỉ có thể hủy yêu cầu đang chờ xử lý.");
        ...
        CancellationToken ct = default)
    {
        ...
        EnsureSupplierReceiptLotsReadyForCompletion(normalized);
        var now = DateTime.UtcNow;
        ...
        return await _unitOfWork.ExecuteInTransactionAsync(async innerCt =>
        {
            ...
            // Thủ kho tạo phiếu là hoàn tất ngay — tồn Kho tăng, không chờ Manager duyệt.
            receipt.SubmittedBy = createdBy;
        ...
        CancellationToken ct = default)
    {
        ...
        if (receipt.Status is not (SupplierReceiptStatus.Draft or SupplierReceiptStatus.Rejected or SupplierReceiptStatus.PendingApproval))
            throw new InventoryValidationException("Chỉ được hoàn tất phiếu nhập ở trạng thái Draft, Rejected hoặc PendingApproval.");
        ...
        return await _unitOfWork.ExecuteInTransactionAsync(async innerCt =>
        {
            var tracked = await _supplierReceiptRepo.GetByIdAsync(id, innerCt)
                ?? throw new InventoryNotFoundException("Không tìm thấy phiếu nhập nhà cung cấp.");

            if (tracked.Status == SupplierReceiptStatus.Completed)
                return MapSupplierReceipt(tracked);

            await ValidateSupplierReceiptItemsForApprovalAsync(tracked, innerCt, catalog);
            tracked.SubmittedBy = actorId;
            tracked.SubmittedAt = DateTime.UtcNow;
            tracked.UpdatedAt = DateTime.UtcNow;
            await ApplySupplierReceiptToWarehouseAsync(
                tracked,
                actorId,
                new CreatorSnapshot(actorId, tracked.CreatedByName, tracked.CreatedByRoleName),
                catalog,
                innerCt);
            await _supplierReceiptRepo.SaveChangesAsync(innerCt);
            return MapSupplierReceipt(tracked);
    ...
    /// <summary>
    /// Trả hàng nhập một bước: Thủ kho tạo phiếu và tồn Kho bị trừ ngay trong cùng transaction,
    /// phiếu chốt luôn ở trạng thái Completed. Không có bước duyệt thứ hai — bằng chứng hậu kiểm
    /// là ảnh hàng lỗi bắt buộc kèm phiếu nhập gốc.
    /// </summary>
        ...
        CancellationToken ct = default)
    {
        ...
        if (request.OperationId == Guid.Empty)
            throw new InventoryValidationException("Thiếu mã thao tác (OperationId).");
        if (request.SupplierReceiptId == Guid.Empty)
            throw new InventoryValidationException("Vui lòng chọn phiếu nhập nhà cung cấp gốc.");

        var defectReasonCode = NormalizeDefectReasonCode(request.DefectReasonCode);
        var evidenceImageUrls = NormalizeEvidenceImageUrls(request.EvidenceImageUrls);
        var reason = NormalizeSnapshotText(request.Reason);
        if (defectReasonCode == DefectReasonOther && string.IsNullOrWhiteSpace(reason))
            throw new InventoryValidationException("Lý do lỗi \"Khác\" phải mô tả chi tiết.");
        if (request.Items == null || request.Items.Count == 0)
            throw new InventoryValidationException("Yêu cầu trả nhà cung cấp phải có ít nhất một dòng SKU.");

        var existing = await _supplierReturnRepo.GetByOperationIdAsync(request.OperationId, ct);
        if (existing != null)
            return MapSupplierReturnRequest(existing);

        var receipt = await _supplierReceiptRepo.GetByIdAsync(request.SupplierReceiptId, ct)
            ?? throw new InventoryValidationException("Không tìm thấy phiếu nhập nhà cung cấp gốc.");

        return await _unitOfWork.ExecuteInTransactionAsync(async innerCt =>
        {
            var today = DateTime.UtcNow.Date;
            var countToday = await _supplierReturnRepo.CountCreatedSinceAsync(today, innerCt);
            var entity = new SupplierReturnRequest
            {
                ...
                SupplierReceiptId = receipt.Id,
                SupplierReceiptCode = receipt.ReceiptCode,
                SupplierName = NormalizeSnapshotText(receipt.SupplierName),
                SupplierReference = NormalizeSnapshotText(receipt.SupplierReference),
                DefectReasonCode = defectReasonCode,
                Reason = reason,
                Note = NormalizeSnapshotText(request.Note),
                Status = InventoryReturnRequestStatus.Completed,
                CreatedBy = createdBy,
                CreatedByName = NormalizeSnapshotText(creator?.CreatedByName),
                CreatedByRoleName = NormalizeSnapshotText(creator?.CreatedByRoleName),
                CreatedAt = DateTime.UtcNow,
                ...
                ReviewedByName = NormalizeSnapshotText(creator?.CreatedByName),
                ReviewedByRoleName = NormalizeSnapshotText(creator?.CreatedByRoleName),
                ReviewedAt = DateTime.UtcNow,
            };

            AddReturnItems(entity, request.Items, LocationWarehouse, innerCt);
            foreach (var (url, index) in evidenceImageUrls.Select((value, i) => (value, i)))
            {
                entity.EvidenceImages.Add(new SupplierReturnEvidenceImage
                {
                    Id = Guid.NewGuid(),
                    SupplierReturnRequestId = entity.Id,
                    ImageUrl = url,
                    SortOrder = index,
            ...
            await ValidateReturnAvailabilityAsync(
                entity.Items.Select(i => (i.SkuId, i.Quantity, i.WarehouseBatchId)),
                LocationWarehouse,
                innerCt);
            ...
            await _supplierReturnRepo.AddAsync(entity, innerCt);
            await _skuStockRepo.SaveChangesAsync(innerCt);
            ...
            foreach (var skuId in touchedSkuIds)
                await PublishLowStockForSkuLocationAsync(skuId, LocationWarehouse, innerCt);

            return MapSupplierReturnRequest(entity);
        }, ct);
    }

    public static List<SupplierReturnDefectReasonResponse> GetSupplierReturnDefectReasons() =>
        SupplierReturnDefectReasonCodes
            .Select(kvp => new SupplierReturnDefectReasonResponse(kvp.Key, kvp.Value))
            .ToList();
    ...
    private static string NormalizeDefectReasonCode(string? code)
    {
        var normalized = NormalizeSnapshotText(code)?.ToUpperInvariant();
        if (string.IsNullOrWhiteSpace(normalized) || !SupplierReturnDefectReasonCodes.ContainsKey(normalized))
            throw new InventoryValidationException("Vui lòng chọn lý do trả hàng hợp lệ.");
        return normalized;
    }

    private static List<string> NormalizeEvidenceImageUrls(List<string>? urls)
    {
        var normalized = new List<string>();
        foreach (var raw in urls ?? [])
        {
            var url = NormalizeSnapshotText(raw);
            if (string.IsNullOrWhiteSpace(url))
                continue;
            if (!url.StartsWith("https://", StringComparison.OrdinalIgnoreCase))
                throw new InventoryValidationException("Đường dẫn ảnh bằng chứng không hợp lệ.");
            if (url.Length > 500)
                throw new InventoryValidationException("Đường dẫn ảnh bằng chứng quá dài.");
            if (!normalized.Contains(url, StringComparer.OrdinalIgnoreCase))
                normalized.Add(url);
        }

        if (normalized.Count == 0)
            throw new InventoryValidationException("Vui lòng đính kèm ảnh hàng lỗi làm bằng chứng hậu kiểm.");
        if (normalized.Count > SupplierReturnRequest.MaxEvidenceImages)
            throw new InventoryValidationException(
                $"Chỉ được đính kèm tối đa {SupplierReturnRequest.MaxEvidenceImages} ảnh bằng chứng.");
        return normalized;
    }
        ...
        CancellationToken ct)
    {
        foreach (var item in items)
        {
            ...
            if (stock.WarehouseQuantityOnHand < item.Quantity)
                throw new InventoryValidationException($"Kho không đủ tồn SKU {stock.SkuCode}.");
        ...
        CancellationToken ct)
    {
        ...
        else if (line.UnitCost is null && item.UnitCost is not null)
        {
        ...
        CancellationToken ct)
    {
        ...
        foreach (var item in check.Items)
        {
            ...
            if (!string.IsNullOrWhiteSpace(item.LotCode) && !SupplierLotCodeRegex.IsMatch(item.LotCode.Trim()))
                throw new InventoryValidationException(
                    $"Mã lô NCC của SKU {item.SkuCode} chỉ được chứa chữ cái, số, dấu gạch ngang (-) và gạch dưới (_).");
            if (string.IsNullOrWhiteSpace(item.LotCode))
                throw new InventoryValidationException($"Mã lô NCC của SKU {item.SkuCode} không được để trống.");
        ...
        CancellationToken ct)
    {
        ...
        foreach (var (line, index) in request.Items.Select((value, i) => (value, i)))
        {
            ...
            if (string.IsNullOrWhiteSpace(lotCode))
                throw new InventoryValidationException($"Dòng {index + 1}: Mã lô NCC không được để trống.");
            if (lotCode.Length > 50)
                throw new InventoryValidationException($"Dòng {index + 1}: Mã lô NCC không được vượt quá 50 ký tự.");
            if (!SupplierLotCodeRegex.IsMatch(lotCode))
                throw new InventoryValidationException(
                    $"Dòng {index + 1}: Mã lô NCC chỉ được chứa chữ cái, số, dấu gạch ngang (-) và gạch dưới (_).");
    ...
    private static void EnsureSupplierReceiptLotsReadyForCompletion(
        IReadOnlyList<NormalizedSupplierReceiptItem> items)
    {
        foreach (var (item, index) in items.Select((value, i) => (value, i)))
        {
            if (string.IsNullOrWhiteSpace(item.LotCode))
                throw new InventoryValidationException($"Dòng {index + 1}: Mã lô NCC không được để trống.");
            if (!SupplierLotCodeRegex.IsMatch(item.LotCode))
                throw new InventoryValidationException(
                    $"Dòng {index + 1}: Mã lô NCC chỉ được chứa chữ cái, số, dấu gạch ngang (-) và gạch dưới (_).");
    ...
    private static SupplierReturnRequestResponse MapSupplierReturnRequest(SupplierReturnRequest request)
    {
        ...
        return new SupplierReturnRequestResponse(
            ...
            request.SupplierReference,
            request.DefectReasonCode,
            SupplierReturnDefectReasonCodes.TryGetValue(request.DefectReasonCode, out var defectLabel) ? defectLabel : request.DefectReasonCode,
            request.EvidenceImages.OrderBy(i => i.SortOrder).Select(i => i.ImageUrl).ToList(),
            request.Reason,
            request.Note,
        ...
        IReadOnlyDictionary<Guid, int>? draftReservedByLine = null)
    {
        ...
        return new(
            ...
            item.AutoProductionOrder?.ProductionCode,
            item.AutoProductionOrder?.Status.ToString());
    }
    ...
    public async Task<ProductionOrderResponse> CreateProductionOrderAsync(
        ...
        CreatorSnapshot? creator,
        CancellationToken ct = default,
        Guid? sourceStockAdjustmentRequestId = null,
        Guid? sourceStockAdjustmentRequestItemId = null)
    {
        ...
        var order = new ProductionOrder
        {
            ...
            CreatedByName = NormalizeSnapshotText(creator?.CreatedByName),
            CreatedByRoleName = NormalizeSnapshotText(creator?.CreatedByRoleName),
            IsAutoGeneratedForShelfReplenishment = sourceStockAdjustmentRequestItemId.HasValue,
            SourceStockAdjustmentRequestId = sourceStockAdjustmentRequestId,
            SourceStockAdjustmentRequestItemId = sourceStockAdjustmentRequestItemId,
            CreatedAt = now,
            UpdatedAt = now,
        ...
        CancellationToken ct = default)
    {
        ...
        if (order.IsAutoGeneratedForShelfReplenishment)
            throw new InventoryValidationException(
                "Lệnh sản xuất tự động của Yêu cầu bổ sung Kệ Hàng không cần gửi duyệt; Warehouse chỉ cần hoàn thành lệnh.");
        ...
        CancellationToken ct = default)
    {
        ...
        if (order.IsAutoGeneratedForShelfReplenishment)
            throw new InventoryValidationException(
                "Lệnh sản xuất tự động của Yêu cầu bổ sung Kệ Hàng không đi qua bước phê duyệt riêng.");
        ...
        CancellationToken ct = default)
    {
        ...
        if (order.IsAutoGeneratedForShelfReplenishment)
            throw new InventoryValidationException(
                "Lệnh sản xuất tự động của Yêu cầu bổ sung Kệ Hàng không được từ chối riêng; kết quả đã được quyết định khi kiểm tra khả năng sản xuất.");
        ...
        CancellationToken ct = default)
    {
        ...
        if (order.IsAutoGeneratedForShelfReplenishment)
            throw new InventoryValidationException(
                "Không thể hủy Lệnh sản xuất tự động đang đáp ứng Yêu cầu bổ sung Kệ Hàng.");
    ...
    private static ProductionOrderResponse MapProductionOrder(ProductionOrder order) => new(
        ...
        order.SourceStockAdjustmentRequestId,
        order.SourceStockAdjustmentRequestItemId);
