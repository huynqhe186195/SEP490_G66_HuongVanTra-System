...
public record StockAdjustmentRequestItemResponse(
    ...
    // Giữ các trường khả dụng cho dữ liệu lịch sử và cơ chế điều chuyển nội bộ.
    int DraftReservedQuantity = 0,
    int AvailableToTransferQuantity = 0,
    Guid? AutoProductionOrderId = null,
    string? AutoProductionOrderCode = null,
    string? AutoProductionOrderStatus = null);
...
public record ProductionOrderResponse(
    ...
    bool IsAutoGeneratedForShelfReplenishment = false,
    Guid? SourceStockAdjustmentRequestId = null,
    Guid? SourceStockAdjustmentRequestItemId = null);

public record ShelfReplenishmentMaterialCheckResponse(
    Guid SkuId,
    string SkuCode,
    string MaterialName,
    int RequiredQuantity,
...
public record ShelfReplenishmentItemProcessResponse(
    Guid RequestId,
    Guid ItemId,
    string RequestCode,
    string Outcome,
    ...
    List<ShelfReplenishmentMaterialCheckResponse> MaterialChecks);
