...
public interface ICustomerTierRepository
{
    ...
    Task<IEnumerable<CustomerTier>> GetAllIncludingInactiveAsync(CancellationToken ct = default);
    Task<CustomerTier?> GetByIdAsync(int id, CancellationToken ct = default);
    Task<CustomerTier?> GetByIdIncludingInactiveAsync(int id, CancellationToken ct = default);
    Task<Dictionary<int, int>> GetCustomerCountsByTierAsync(CancellationToken ct = default);
