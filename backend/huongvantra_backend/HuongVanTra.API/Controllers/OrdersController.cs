...
namespace HuongVanTra.API.Controllers {
    ...
    public class OrdersController : ApiControllerBase {
        ...
        /// <summary>Poll trạng thái thanh toán (chờ webhook SePay sau khi khách quét QR).</summary>
        [HttpGet("{id:int}/payment-status")]
        public async Task<ActionResult<OrderPaymentStatusDto>> GetPaymentStatus(
            int id,
            CancellationToken cancellationToken = default) {
            var access = await ResolveOrderAccessAsync(cancellationToken);
            var status = await _orderService.GetOrderPaymentStatusAsync(id, access, cancellationToken);
            if (status is null) {
                return NotFound("Order not found.");
            }
